...
function FloorGo(domId){//传入目标的id
  var lifeServiceTop=$('#'+domId+'').offset().top-150;//获取目标的滚动条高度
  var scroChange=setInterval(function(){
    var win_scT=$(window).scrollTop();//获取当前滚动条高度
    var scT_pc=lifeServiceTop-win_scT;//获取目标与当前滚动条的差值
    if(scT_pc<0){//如果差值小于0；表示目标在目前滚动条的上方
      var scT_change1=Math.abs(scT_pc/3);
      $(window).scrollTop(win_scT-scT_change1);
      var pcz=Math.abs(win_scT-scT_change1-lifeServiceTop);
